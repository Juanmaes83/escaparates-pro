...
test('Museum Phase 2 closure — integration, UX hardening and reversible runtime',async({page})=>{
  ...
  await page.locator('[data-p2-return]').click();await expect(page.locator('#st')).toBeVisible();await expect(page.locator('[data-domain="visitor"]')).toHaveClass(/is-on/);
  ...
  // BUILDER: room accessibility provides real route semantics.
  ...
  // EXPERIENCE WORKSPACE V2: full controls visible, no horizontal clipping, route uses room data.
  ...
  await page.locator('[data-p2-calc-route]').click();await expect(page.locator('.p2-route')).toBeVisible();
  ...
  await page.locator('[data-domain="publish"]').click();await expect(page.getByRole('heading',{name:'Publicar',exact:true})).toBeVisible();
