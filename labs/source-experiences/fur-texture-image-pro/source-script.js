document.addEventListener("DOMContentLoaded", () => {
  ...
  const furFolder = pane.addFolder({ title: "Fur Settings" });
  folders.push(furFolder);
  observeFolder(furFolder);
  furFolder.addInput(params, "hairDensity", {
    min: 1,
    max: 10,
    step: 1,
    label: "Hairs/Pixel"
  ...
  processButtonPane.on("click", () => {
    if (imgSRC) {
      setFolderEnabled(furFolder, false);
  ...
  function stopProcess() {
    return new Promise((resolve) => {
      if (isProcessing) {
        ...
        setFolderEnabled(furFolder, true);
        isProcessing = false;
  ...
  function cleanupMemory() {
    ...
    if (imageData) {
      delete imageData.data; // Fully remove pixel data
      imageData = null;
    }
  }
  ...
  function processImage() {
    ...
    function processPixel(index) {
      ...
      for (let i = 0; i < params.hairDensity; i++) {
        ...
        let length = baseLength + (prng() > 0.5 ? 1 : 0.5);
        ...
        drawHairOptimized(
          ...
          alpha,
          [red + colorVariation, green + colorVariation, blue + colorVariation]
        );
      }
    }
  ...
  function drawHairOptimized(x, y, length, rotation, alpha, color) {
  ...
  function drawHair(x, y, length, rotation, alpha, color) {
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate((rotation * Math.PI) / 180);
    ctx.globalAlpha = alpha;
    ctx.strokeStyle = `rgb(${color[0]}, ${color[1]}, ${color[2]})`;
    ctx.lineWidth = 1; // Thin lines for hair-like appearance
    ctx.beginPath();
    ctx.moveTo(0, 0);
    ctx.lineTo(length, 0); // Draw a line to simulate a hair
    ctx.stroke();
    ctx.restore();
  }
  ...
  function finishProcessing() {
    ...
    processedCtx.fillStyle = "#1d1d1f"; // Change to any color you want or transparent
