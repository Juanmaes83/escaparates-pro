 ...
 * a real artwork from the Museum collection as a living painting inside a
 * Museum room.
 *
...
const DEFAULT_ARTWORK = 'campo-de-ceniza.jpg';
const COLLECTION_BASE = '../../../assets/collection/';
const ART_SIZE = 256;
const RT_SIZE = 1024;
...
export class LivingArtGuest {
  ...
  async activate({ config = {} } = {}) {
    ...
    const artworkName = config.artwork || DEFAULT_ARTWORK;
    let artResult;
    try {
      artResult = await this._loadArtworkImage(artworkName);
    } catch (e) {
      artResult = {
        pixels: this._generateProceduralArtwork(ART_SIZE, ART_SIZE),
        width: ART_SIZE,
        height: ART_SIZE,
    ...
    this._artworkSource = artResult.source;
    await this._buildPipeline(artResult.pixels, artResult.width, artResult.height);
  ...
  report() {
    return {
      ...
      artworkSource: this._artworkSource,
      capabilities: [
  ...
  _loadArtworkImage(filename) {
    return new Promise((resolve, reject) => {
      const img = new Image();
      img.crossOrigin = 'anonymous';
      img.onload = () => {
        const w = Math.min(img.naturalWidth, ART_SIZE);
        const h = Math.min(img.naturalHeight, ART_SIZE);
      ...
      img.onerror = () => reject(new Error('Failed to load artwork: ' + filename));
      img.src = COLLECTION_BASE + filename;
    });
  }
