...
async function waitReady(timeout = 14000) {
  const start = performance.now();
  while (performance.now() - start < timeout) {
    if (bridgeReady && iframe()?.contentWindow) return true;
    await sleep(80);
  }
  return false;
}

function request(type, payload = {}, timeout = 16000) {
...
function cleanState(state) {
  ...
  return {
    ...
    autoRotate: Boolean(state.autoRotate), runSimulation: state.runSimulation !== false, wireframe: Boolean(state.wireframe),
...
function stable(value) {
  ...
  return Object.fromEntries(Object.keys(value).filter((k) => value[k] !== undefined).sort().map((k) => [k, stable(value[k])]));
}
...
function evidence(studio) {
  const ledger = loadLedger(); const row = ledger.pieces?.[ENTITY_ID] || {}; const hash = pieceHash(studio);
  return { hash, preview: row.previewHash === hash, saved: row.savedHash === hash, validated: row.validatedHash === hash };
}
function writeEvidence(studio, level) {
  const ledger = loadLedger(); const hash = pieceHash(studio); const now = new Date().toISOString();
...
function invalidate(studio) {
  const ledger = loadLedger(); if (ledger.pieces) delete ledger.pieces[ENTITY_ID]; ledger.route = {}; saveLedger(ledger);
  studio.dirty = true;
}
...
window.addEventListener('message', (event) => {
  ...
  if (msg.type === 'BREEZE_BRIDGE_READY') {
    bridgeReady = true; liveState = msg.state || liveState;
    const studio = window.__IW_STUDIO;
    if (studio && liveState) syncConfig(studio, liveState, false);
    if (savedState && iframe()?.contentWindow) iframe().contentWindow.postMessage({ bridge: BRIDGE, type: 'BREEZE_APPLY_STATE', requestId: `restore-${Date.now()}`, state: savedState }, '*');
  }
  if (msg.type === 'BREEZE_STATE_CHANGED') {
    liveState = { ...(liveState || {}), ...(msg.state || {}) };
    const studio = window.__IW_STUDIO;
  ...
  if (msg.requestId && pending.has(msg.requestId)) {
    ...
    if (msg.type === 'BREEZE_BRIDGE_ERROR') p.reject(new Error(msg.error || 'Breeze bridge error'));
    else p.resolve(msg);
  }
});
...
StudioShell.prototype._validationPreviewPiece = async function breezePreview() {
  ...
  try {
    await this.onReveal(ENTITY_ID); bridgeReady = false;
    await sleep(180);
    ...
    const check = await request('BREEZE_VALIDATE');
    if (!check.result?.ok) throw new Error('La sala Breeze no confirma todos sus subsistemas.');
    writeEvidence(this, 'preview');
    this._say('PREVIEW OK · Breeze Studio visible y operativo. Cambios de fondo, pañuelo, escultura y efectos se están leyendo desde la sala.');
  } catch (error) { this._say(`PREVIEW CON ERROR · ${String(error?.message || error)}`, true); }
  finally { this.busy = null; this.render(); }
};
...
StudioShell.prototype._validationSavePiece = async function breezeSave() {
  ...
  try {
    const stateMsg = await request('BREEZE_GET_STATE');
    liveState = stateMsg.state; savedState = stateMsg.state; syncConfig(this, savedState, false);
    ConfigStore.save(this.config); this.dirty = false; this.savedAt = new Date(); writeEvidence(this, 'saved');
    window.__IW_BREEZE_SAVED_STATE = savedState;
    this._say('BREEZE GUARDADO · experiencia, fondo, pañuelo, escultura y física quedan guardados para esta sesión. Falta VALIDAR PIEZA.');
    this.render();
...
StudioShell.prototype._validationValidatePiece = async function breezeValidate() {
  ...
  try {
    await request('BREEZE_APPLY_STATE', { state: savedState }, 24000);
    const check = await request('BREEZE_VALIDATE');
    if (!check.result?.ok) throw new Error('Breeze no pudo restaurar/confirmar todos sus subsistemas.');
    syncConfig(this, savedState, false); writeEvidence(this, 'validated');
    const ok = Object.entries(check.result.checks || {}).filter(([,v]) => v).map(([k]) => k).join(', ');
...
window.__IW_BREEZE_AUTHORING_BRIDGE = {
  entityId: ENTITY_ID,
