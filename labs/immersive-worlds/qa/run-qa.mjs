...
async function main() {
  ...
  try {
    ...
    // Resume mode.
    //
    // A long suite that can only start from zero forces a full replay whenever a
    // run dies late — which is how ninety minutes were spent re-proving 51 checks
    // that a completed, audited run had already proven. With IW_QA_TAIL=1 the
    // suite boots straight into the gallery and runs only the closing sections,
