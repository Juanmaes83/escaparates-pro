...
description: Repository-wide Escaparates Pro operating contract for long-running autonomous engineering. Governs Claude Code, Codex and comparable agents across every module. Requires additive, isolated, reversible, evidence-driven, learning execution; safe cloning; source-of-truth protection; explicit checkpoints and human gates; no merge or canonical promotion without Juanma.
---
...
- run QA, diagnose, fix, rerun, document and continue through authorized internal checkpoints.
...
Do not wait for routine permission. Diagnose → fix → targeted validation → rerun → document → learn → continue.
...
During a measurement/QA run, do not mutate:
...
## CHEAP VALIDATION → EXPENSIVE FULL QA
...
Do not repeatedly pay for a long full suite before confirming the intended fix cheaply. But targeted pass is not final closure; the full appropriate gate remains mandatory.

## PROTECTED BASELINE + ROLLBACK PROOF

Protected baseline means new work must fit around it. Do not change the baseline until the new work passes.
...
9. targeted/full QA + run IDs;
10. failures/fixes/ledger updates/lessons;
11. visual/technical evidence + preview route;
12. limitations;
13. branch health + justification;
14. canonical docs updated yes/no;
15. remaining authorized scope;
16. stop reason;
17. next action;
18. next human decision;
19. explicit `MERGE_PERFORMED: NO`;
20. STOP.
