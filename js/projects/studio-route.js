(function(){
  ...
  function tabFor(id){
    ...
    return Array.from(document.querySelectorAll('.tab')).find(function(button){
  ...
  function addProductNavigation(){
    ...
    projects.id='studioProjectLibrary';
    projects.className='btn';
    projects.href='projects.html';
    projects.textContent='Biblioteca';
    projects.setAttribute('aria-label','Abrir la biblioteca de proyectos');
    toolbar.appendChild(projects);
  }
  ...
  function waitForList(projectName,tries){
    ...
    var row=rows.find(function(item){
      ...
      return title&&title.textContent===projectName;
    });
    if(row){
      var button=Array.from(row.querySelectorAll('button')).find(function(item){
        return item.textContent.trim()==='Abrir';
      });
