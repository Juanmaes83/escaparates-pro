...
function label(id){return{'real-estate-storytelling-custom-pro':'Real Estate Storytelling','product-storytelling-custom-pro':'Product Storytelling','luxury-real-estate-custom-pro':'Luxury Real Estate'}[id]||id||'Custom PRO'}
...
async function act(action,p){
  try{
    if(action==='duplicate'){
      var copy=JSON.parse(JSON.stringify(p));delete copy.id;delete copy.cloudId;delete copy.revision;copy.name=(p.name||'Proyecto')+' — copia';copy.status='draft';copy.published=null;await EP.ProjectStoreLocal.save(copy);setStatus('Proyecto duplicado localmente ✓');
