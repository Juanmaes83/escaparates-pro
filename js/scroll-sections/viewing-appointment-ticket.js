// Viewing Appointment Ticket — adapted from the CodePen gist "[Tailwind]
// E-Ticket" (source read & understood: a boarding-pass-style card split into
// a main stub and a perforated ticket stub, the perforation drawn with
// radial-gradient "bite" masks on facing corners plus a dashed divider line,
// a staggered fade/slide-in for each field, a small plane icon that flies in
// across the header, and an SVG barcode). Rebuilt in plain CSS (the source
// leans on Tailwind's JIT arbitrary-variant syntax, which needs the Tailwind
// CDN at runtime — dropped in favour of a static stylesheet so the exported
// file has zero external framework dependency) and reframed as a property
// viewing appointment confirmation instead of a flight boarding pass.
(function() {
    function build(mediaList, opts) {
        opts = opts || {};
        var title = opts.title || 'Escaparate';
        var propertyName = opts.propertyName || title;
        var address = opts.address || 'Calle Mayor 12, Madrid';
        var date = opts.date || '18 Jul';
        var time = opts.time || '17:30';
        var agent = opts.agent || 'M. García';
        var reference = opts.reference || 'REF-2043';
...
'<title>' + title + ' — Ticket de Visita</title>\n' +
'<style>\n' +
...
'main{--r:10px;--bg:#ECFEFF;color:#94a3b8;font-weight:300;width:100%;max-width:56rem;display:grid;grid-template-columns:4fr 1fr;background:var(--bg);border-radius:14px;overflow:hidden;box-shadow:0 30px 80px rgba(0,0,0,0.35);}\n' +
'@media (max-width:640px){main{grid-template-columns:1fr;max-width:24rem;}}\n' +
'.vt-main{padding:1.75rem 2rem;position:relative;background:radial-gradient(circle var(--r) at top right,#0000 98%,var(--bg)) top right,radial-gradient(circle var(--r) at bottom right,#0000 98%,var(--bg)) bottom right;background-size:100% 51%;background-repeat:no-repeat;}\n' +
'@media (max-width:640px){.vt-main{background:radial-gradient(circle var(--r) at bottom left,#0000 98%,var(--bg)) bottom left,radial-gradient(circle var(--r) at bottom right,#0000 98%,var(--bg)) bottom right;background-size:51% 100%;background-repeat:no-repeat;}}\n' +
'.vt-main::before{content:"";position:absolute;border:2px dashed #ddd;top:calc(var(--r) * 3);right:-1px;bottom:auto;left:auto;height:calc(100% - var(--r) * 6);width:0;}\n' +
...
'.vt-stub{display:grid;place-content:center;padding:1.25rem;background:var(--bg);}\n' +
...
'  <div class="vt-main">\n' +
'    <div class="vt-route">\n' +
'      <h2>' + propertyName.slice(0, 3).toUpperCase() + '</h2>\n' +
'      <div class="vt-plane"><span>&#9992;</span></div>\n' +
'      <h2>VISITA</h2>\n' +
'    </div>\n' +
'    <div class="vt-labels"><span>' + propertyName + '</span><span>' + address + '</span></div>\n' +
'    <div class="vt-fields">\n' +
'      <div><h3>Fecha</h3><time>' + date + '</time></div>\n' +
'      <div><h3>Hora</h3><time>' + time + '</time></div>\n' +
'      <div><h3>Agente</h3><time>' + agent + '</time></div>\n' +
'      <div><h3>Referencia</h3><time>' + reference + '</time></div>\n' +
'    </div>\n' +
'    <p class="vt-note">Presenta este ticket (impreso o en tu móvil) al llegar a la visita. Cualquier cambio de horario, contacta con tu agente.</p>\n' +
'  </div>\n' +
'  <div class="vt-stub">\n' +
'    <div class="vt-stub-inner">\n' +
'      <div class="vt-stub-route"><h2>' + propertyName.slice(0, 3).toUpperCase() + '</h2><span>&#9992;</span><h2>VISITA</h2></div>\n' +
'      <div class="vt-stub-fields"><div>Fecha<time>' + date + '</time></div><div>Hora<time>' + time + '</time></div><div>Ref.<time>' + reference + '</time></div></div>\n' +
...
'</html>\n';
    ...
    EP.ScrollSections.register({
        ...
        name: 'Ticket de Visita Concertada',
        icon: '🎫',
        description: 'Tarjeta estilo tarjeta de embarque para confirmar una visita a una propiedad — recorte perforado real entre el cuerpo y el resguardo, campos de fecha/hora/agente/referencia con entrada escalonada, y código de barras decorativo',
