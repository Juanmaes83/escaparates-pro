...
function runtimeScript(o){
  ...
  return '(function(){var $=function(s){return document.querySelector(s)},$$=function(s){return [].slice.call(document.querySelectorAll(s))};var data=JSON.parse($("#rsData").textContent),products=data.products,modal=$("#rsModal"),runway=$("#rsRunway"),floating=$("#rsFloating"),progress=$("#rsProgress"),page=$(".rs-page"),menu=$("#rsMenu"),panel=$("#rsNavPanel"),panelClose=$("#rsPanelClose"),langButton=$("#rsLanguage"),audio=$("#rsAudio"),cursor=$("#rsCursor"),trackShell=$("#rsTrackShell"),runwayTimer=null,runwayIndex=0,wishlist=JSON.parse(localStorage.getItem("rsWishlist")||"[]"),cart=JSON.parse(localStorage.getItem("rsCart")||"[]"),raf=0,cursorRaf=0,scrollDirty=true,timers=[],lastFocus=null,langKey="ep:fashion-commerce:language",langUserKey="ep:fashion-commerce:language:user",savedLang=localStorage.getItem(langKey),lang=(localStorage.getItem(langUserKey)==="1"&&data.i18n[savedLang]?savedLang:(data.language||"es"));if(!data.i18n[lang])lang=data.language||"es";function later(fn,ms){var id=setTimeout(fn,ms);timers.push(id);return id}function money(v){try{return new Intl.NumberFormat(data.priceLocale||"es-ES",{style:"currency",currency:data.currency||"EUR"}).format(Number(v)||0)}catch(e){return (data.currency||"EUR")+" "+v}}function reduced(){return document.body.classList.contains("motion-reduced")||(window.matchMedia&&window.matchMedia("(prefers-reduced-motion: reduce)").matches)}function state(next){if(page)page.dataset.state=next}function pack(){return data.i18n[lang]||data.i18n.es}function spanify(value){return String(value||"").split("").map(function(char){return "<span class=\\"rs-glitch-char\\">"+char.replace(/[&<>]/g,function(c){return {"&":"&amp;","<":"&lt;",">":"&gt;"}[c]})+"</span>"}).join("")}function applyLang(next,persist){if(next&&data.i18n[next])lang=next;var p=pack();document.documentElement.lang=lang;page.dataset.language=lang;if(persist){localStorage.setItem(langKey,lang);localStorage.setItem(langUserKey,"1")}if(langButton)langButton.textContent=lang.toUpperCase();$$("[data-i18n]").forEach(function(el){var key=el.dataset.i18n;if(p[key]!=null)el.textContent=p[key]});$$("[data-i18n-aria]").forEach(function(el){var key=el.dataset.i18nAria;if(p[key]!=null)el.setAttribute("aria-label",p[key])});$$("[data-i18n-placeholder]").forEach(function(el){var key=el.dataset.i18nPlaceholder;if(p[key]!=null)el.setAttribute("placeholder",p[key])});$$("[data-i18n-glitch]").forEach(function(el){var key=el.dataset.i18nGlitch;if(p[key]!=null){el.innerHTML=spanify(p[key]);el.setAttribute("aria-label",p[key])}});if(runway)runway.textContent=runwayTimer?p.runwayStopLabel:p.runwayLabel}function scrollToId(id){var el=document.getElementById(id);if(el)el.scrollIntoView({behavior:reduced()?"auto":"smooth"})}function closeMenu(){if(!page)return;page.classList.remove("nav-open");if(panel)panel.setAttribute("aria-hidden","true");if(menu)menu.setAttribute("aria-expanded","false");if(lastFocus&&document.contains(lastFocus))lastFocus.focus()}function openMenu(){lastFocus=document.activeElement;if(!page)return;page.classList.add("nav-open");if(panel)panel.setAttribute("aria-hidden","false");if(menu)menu.setAttribute("aria-expanded","true");var first=panel&&panel.querySelector("button,a");if(first)first.focus()}function toggleMenu(){page.classList.contains("nav-open")?closeMenu():openMenu()}$$("[data-scroll]").forEach(function(b){b.addEventListener("click",function(){closeMenu();scrollToId(b.dataset.scroll)})});if(menu)menu.addEventListener("click",toggleMenu);if(panelClose)panelClose.addEventListener("click",closeMenu);document.addEventListener("keydown",function(e){if(e.key==="Escape"){closeMenu();closeProduct()}if(e.key==="Tab"&&page.classList.contains("nav-open")&&panel){var focusable=$$("button,a").filter(function(el){return panel.contains(el)&&!el.disabled});if(focusable.length){var first=focusable[0],last=focusable[focusable.length-1];if(e.shiftKey&&document.activeElement===first){e.preventDefault();last.focus()}else if(!e.shiftKey&&document.activeElement===last){e.preventDefault();first.focus()}}}});function setActive(){var sections=["section0","section1","section2","section3"],active=sections[0];sections.forEach(function(id){var el=document.getElementById(id);if(el&&el.getBoundingClientRect().top<innerHeight*.45)active=id});$$("[data-nav-target]").forEach(function(item){item.classList.toggle("active",item.dataset.navTarget===active)})}function openProduct(i){var p=products[i];if(!p)return;var card=$$("[data-product-index]")[i],img=card&&card.querySelector("img");$("#rsModalMedia").innerHTML=img?\'<img src="\'+img.src+\'" alt="\'+p.name.replace(/"/g,"&quot;")+\'">\':"";$("#rsModalEyebrow").textContent=p.eyebrow||p.category||"";$("#rsModalTitle").textContent=p.name;$("#rsModalDescription").textContent=p.description||"";$("#rsModalPrice").textContent=money(p.price);$("#rsWishlist").onclick=function(){wishlist.push(p.id);localStorage.setItem("rsWishlist",JSON.stringify(wishlist));toast(pack().wishlistToast+p.name)};$("#rsCart").onclick=function(){cart.push(p.id);localStorage.setItem("rsCart",JSON.stringify(cart));toast(pack().cartToast+p.name)};modal.classList.add("open");modal.setAttribute("aria-hidden","false");$("#rsClose").focus()}function closeProduct(){if(!modal)return;modal.classList.remove("open");modal.setAttribute("aria-hidden","true")}$$("[data-open-product]").forEach(function(b){b.addEventListener("click",function(){openProduct(Number(b.dataset.openProduct))})});$("#rsClose").addEventListener("click",closeProduct);modal.addEventListener("click",function(e){if(e.target===modal)closeProduct()});function setRunway(i){var cards=$$("[data-product-index]");cards.forEach(function(card,n){card.classList.toggle("runway-active",n===i)});if(cards[i])cards[i].scrollIntoView({behavior:reduced()?"auto":"smooth",inline:"center",block:"nearest"})}function stopRunway(){clearInterval(runwayTimer);runwayTimer=null;runway.textContent=pack().runwayLabel}function startRunway(){stopRunway();runway.textContent=pack().runwayStopLabel;setRunway(runwayIndex);runwayTimer=setInterval(function(){runwayIndex=(runwayIndex+1)%products.length;setRunway(runwayIndex)},'+runwayMs+')}runway.addEventListener("click",function(){runwayTimer?stopRunway():startRunway()});function draw(){raf=0;scrollDirty=false;var max=document.documentElement.scrollHeight-innerHeight;progress.style.width=(max>0?Math.min(100,scrollY/max*100):0)+"%";floating.classList.toggle("show",scrollY>innerHeight*.7);setActive()}function requestDraw(){scrollDirty=true;if(!raf)raf=requestAnimationFrame(draw)}function moveCursor(e){if(!cursor||reduced())return;var over=trackShell&&trackShell.contains(e.target);cursor.classList.toggle("drag",over);if(!cursorRaf)cursorRaf=requestAnimationFrame(function(){cursorRaf=0;cursor.style.left=e.clientX+"px";cursor.style.top=e.clientY+"px"})}function pulseGlitch(){if(!page||reduced())return;page.classList.add("glitch-pulse");later(function(){page.classList.remove("glitch-pulse")},180)}function setupMagnet(){if(reduced())return;$$(".rs-nav-cta,.rs-floating,.rs-gallery-head button,.rs-card-copy button").forEach(function(el){el.addEventListener("pointermove",function(e){var r=el.getBoundingClientRect(),x=(e.clientX-r.left-r.width/2)*.16,y=(e.clientY-r.top-r.height/2)*.16;el.style.transform="translate("+x+"px,"+y+"px)"});el.addEventListener("pointerleave",function(){el.style.transform=""})})}window.addEventListener("scroll",requestDraw,{passive:true});window.addEventListener("resize",requestDraw);window.addEventListener("pointermove",moveCursor,{passive:true});floating.addEventListener("click",function(){scrollToId("section1")});if(audio)audio.addEventListener("click",function(){var on=audio.getAttribute("aria-pressed")!=="true";audio.setAttribute("aria-pressed",String(on));audio.textContent=on?"SOUND":"MUTE";page.classList.toggle("audio-on",on)});if(langButton)langButton.addEventListener("click",function(){applyLang(lang==="es"?"en":"es",true)});function toast(text){var el=document.createElement("div");el.textContent=text;el.style.cssText="position:fixed;z-index:220;left:50%;bottom:26px;transform:translateX(-50%);background:#ff5c00;color:#050505;padding:12px 18px;font-family:Space Mono,monospace;font-size:12px";document.body.appendChild(el);later(function(){el.remove()},1800)}function runIntro(){var teaser=$("#rsTeaser"),pre=$("#rsPreloader"),preText=$("#rsPreloaderText"),word="'+esc(preloader)+'",i=0;if(reduced()||(!teaser&&!pre)){state("skipped");later(function(){state("ready")},0);return}if(teaser){state("teaser");later(function(){teaser.classList.add("hide");showPreloader()},'+teaserMs+')}else showPreloader();function showPreloader(){if(!pre){state("ready");return}state("preloader");type()}function type(){if(!preText){state("ready");return}if(i<=word.length){preText.textContent=word.slice(0,i++);later(type,70)}else later(function(){pre.classList.add("hide");state("ready")},450)}}window.addEventListener("pagehide",cleanup);window.addEventListener("beforeunload",cleanup);function cleanup(){stopRunway();if(raf)cancelAnimationFrame(raf);if(cursorRaf)cancelAnimationFrame(cursorRaf);timers.forEach(clearTimeout);window.removeEventListener("scroll",requestDraw);window.removeEventListener("resize",requestDraw);window.removeEventListener("pointermove",moveCursor)}applyLang(lang,false);setupMagnet();later(pulseGlitch,900);later(pulseGlitch,2600);requestDraw();runIntro()})();';
}
