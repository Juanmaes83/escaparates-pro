...
function mediaFor(media,id,index,fallback,type){var item=media.filter(function(candidate){return candidate.slot===id;})[0]||media[index]||null;return item&&item.url?item:{slot:id,type:type||'image',url:fallback||''};}
function products(value){return (Array.isArray(value)?value:DEFAULTS.products).slice(0,12).map(function(item,index){return Object.assign({},DEFAULTS.products[index%DEFAULTS.products.length],item||{});});}
...
function build(mediaInput,raw){
  ...
  var heroVideo=mediaFor(media,'heroVideo',0,RUBIK_MEDIA.heroVideo,'video');
  var heroPoster=mediaFor(media,'heroPoster',1,RUBIK_MEDIA.heroPoster,'image');
  var gallery=list.map(function(product,index){
    var image=mediaFor(media,'product'+(index+1)+'Primary',index+2,RUBIK_MEDIA['product'+((index%8)+1)],'image');
  ...
  var videos=['campaignVideo1','campaignVideo2','campaignVideo3','campaignVideo4'].map(function(slot,index){
    return '<article class="rs-video-tile">'+mediaVideo(mediaFor(media,slot,20+index,RUBIK_MEDIA[slot],'video'),'rs-video','')+'<span>0'+(index+1)+'</span></article>';
  }).join('');
  var dataJson=JSON.stringify({products:list,currency:o.currency,priceLocale:o.priceLocale,mediaSlots:MEDIA_SLOTS.map(function(slot){return slot.id;})}).replace(/</g,'\\u003c');
  return '<!doctype html><html lang="'+attr(o.language||'es')+'"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>'+esc(o.brand)+' | '+esc(o.collectionName)+'</title><link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Bebas+Neue&family=Inter:wght@300;400;700;900&family=Space+Mono:wght@400;700&display=swap" rel="stylesheet"><style>'+css(o)+'</style></head><body><div class="rs-page" data-template="fashion-commerce-pro" data-preset="'+attr(o.presetId)+'"><div class="rs-progress" id="rsProgress"></div>'+(bool(o.teaserEnabled,true)?'<section class="rs-teaser" id="rsTeaser" aria-label="Teaser"><div><span></span><h1>'+esc(o.brand)+'</h1><span></span><p>'+esc(o.volumeLabel)+'</p></div></section>':'')+(bool(o.preloaderEnabled,true)?'<section class="rs-preloader" id="rsPreloader"><strong id="rsPreloaderText"></strong></section>':'')+'<nav class="rs-nav"><strong>'+esc(o.brand)+'</strong><div>'+nav+'</div><a href="'+attr(safeUrl(o.heroCtaUrl,'#section1'))+'">'+esc(o.heroCtaLabel)+'</a></nav><main><section class="rs-hero" id="section0">'+mediaVideo(heroVideo,'rs-hero-video',heroPoster.url)+'<div class="rs-hero-overlay"></div><div class="rs-hero-copy"><span>'+esc(o.heroEyebrow)+'</span><h1 class="rs-glitch" aria-label="'+attr(o.heroTitle)+'">'+glitch+'</h1><p>'+esc(o.heroSubtitle)+'</p></div></section><section class="rs-gallery" id="section1"><div class="rs-gallery-head"><div><span>01 / '+esc(o.galleryEyebrow)+'</span><h2>'+esc(o.galleryTitle)+'</h2></div><button type="button" id="rsRunway">'+esc(o.runwayLabel)+'</button></div><div class="rs-track-shell"><div class="rs-track" id="rsTrack">'+gallery+'</div></div></section><section class="rs-lookbook" id="section2"><span>02 / LOOKBOOK</span><h2>'+esc(o.lookbookTitle)+'</h2><p>'+esc(o.lookbookIntro)+'</p><button type="button" data-scroll="section1">'+esc(o.lookbookCtaLabel)+'</button></section><section class="rs-videos" id="section3"><span>03 / CAMPAIGN FILMS</span><div class="rs-video-grid">'+videos+'</div></section></main><button class="rs-floating" type="button" id="rsFloating">'+esc(o.floatingCtaLabel)+'</button><section class="rs-modal" id="rsModal" aria-hidden="true" role="dialog" aria-modal="true"><div class="rs-modal-card"><button type="button" id="rsClose" aria-label="Cerrar">×</button><div id="rsModalMedia"></div><div><span id="rsModalEyebrow"></span><h2 id="rsModalTitle"></h2><p id="rsModalDescription"></p><strong id="rsModalPrice"></strong><div class="rs-modal-actions"><button type="button" id="rsWishlist">'+esc(o.wishlistLabel)+'</button><button type="button" id="rsCart">'+esc(o.cartLabel)+'</button></div><small>'+esc(o.demoDisclaimer)+'</small></div></div></section><script type="application/json" id="rsData">'+dataJson+'</script><script>'+runtimeScript(o)+'</script></div></body></html>';
}

function css(o){
  var motion=o.motionProfile&&typeof o.motionProfile==='object'?o.motionProfile:{intensity:70,duration:650,reducedMotion:false};
  return ':root{--orange:'+attr(o.accentColor)+';--black:'+attr(o.backgroundColor)+';--white:'+attr(o.textColor)+';--gray:#151515;--line:rgba(255,255,255,.18);--dur:'+clamp(motion.duration,0,5000,650)+'ms}*{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;background:var(--black);color:var(--white);font-family:Inter,Arial,sans-serif;overflow-x:hidden}button,a{font:inherit}.rs-page{background:var(--black);min-height:100vh}.rs-progress{position:fixed;z-index:200;left:0;top:0;height:3px;width:0;background:var(--orange)}.rs-teaser,.rs-preloader{position:fixed;z-index:180;inset:0;background:#000;display:grid;place-items:center;text-align:center;transition:opacity .55s,visibility .55s}.rs-teaser.hide,.rs-preloader.hide{opacity:0;visibility:hidden;pointer-events:none}.rs-teaser h1,.rs-preloader strong{font-family:"Bebas Neue",Impact,sans-serif;font-size:clamp(48px,9vw,112px);letter-spacing:.18em;margin:0}.rs-teaser span{display:block;height:2px;width:min(420px,70vw);background:var(--orange);margin:12px auto}.rs-teaser p,.rs-hero-copy span,.rs-gallery-head span,.rs-lookbook span,.rs-videos span,.rs-card-copy span,.rs-modal-card span{font-family:"Space Mono",monospace;text-transform:uppercase;letter-spacing:.22em;font-size:11px;color:var(--orange)}.rs-preloader strong{border-right:3px solid var(--orange);padding-right:12px}.rs-nav{position:fixed;z-index:90;top:0;left:0;right:0;height:72px;display:flex;align-items:center;gap:24px;padding:0 28px;background:rgba(0,0,0,.62);backdrop-filter:blur(14px);border-bottom:1px solid var(--line)}.rs-nav strong{font-family:"Bebas Neue";font-size:26px;letter-spacing:.1em;margin-right:auto}.rs-nav div{display:flex;gap:8px;flex-wrap:wrap}.rs-nav-link,.rs-nav a{background:none;border:0;color:rgba(255,255,255,.75);text-decoration:none;text-transform:uppercase;font-family:"Space Mono";font-size:11px;letter-spacing:.12em;cursor:pointer}.rs-nav a{border:1px solid var(--orange);padding:10px 14px;color:#fff}.rs-hero{height:100vh;position:relative;display:grid;place-items:center;overflow:hidden;text-align:center}.rs-hero-video{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;filter:grayscale(1) contrast(1.32)}.rs-hero-overlay{position:absolute;inset:0;background:radial-gradient(circle at center,rgba(255,92,0,.06),rgba(0,0,0,.78) 72%)}.rs-hero-copy{position:relative;z-index:2;padding:0 18px}.rs-glitch{display:flex;justify-content:center;flex-wrap:wrap;font-family:"Bebas Neue";font-size:clamp(68px,15vw,210px);line-height:.78;letter-spacing:.01em;margin:18px 0;text-shadow:4px 4px 0 rgba(0,0,0,.82)}.rs-glitch-char{display:inline-block;transition:transform .18s,color .18s}.rs-glitch-char:hover{color:var(--orange);transform:translate(3px,-3px) skew(-8deg)}.rs-hero-copy p{font-family:"Space Mono";font-size:clamp(12px,1.5vw,18px);letter-spacing:.28em}.rs-gallery{min-height:100vh;padding:104px 0;background:#030303}.rs-gallery-head{display:flex;align-items:end;justify-content:space-between;padding:0 5vw 28px;gap:24px}.rs-gallery-head h2,.rs-lookbook h2{font-family:"Bebas Neue";font-size:clamp(52px,8vw,132px);line-height:.82;margin:12px 0 0}.rs-gallery-head button,.rs-lookbook button,.rs-card-copy button,.rs-modal-actions button{background:var(--orange);color:#050505;border:0;padding:13px 17px;font-family:"Bebas Neue";font-size:18px;letter-spacing:.06em;cursor:pointer}.rs-track-shell{overflow:auto;padding:0 5vw 28px;scrollbar-color:var(--orange) #222}.rs-track{display:flex;gap:18px;width:max-content}.rs-card{width:min(390px,82vw);background:#111;border:1px solid var(--line)}.rs-media-button{position:relative;display:block;width:100%;aspect-ratio:3/4;border:0;padding:0;overflow:hidden;background:#222;cursor:pointer}.rs-card-img{width:100%;height:100%;object-fit:cover;filter:grayscale(1) contrast(1.18);transition:filter var(--dur),transform var(--dur)}.rs-card:hover .rs-card-img,.rs-card.runway-active .rs-card-img{filter:grayscale(0) contrast(1);transform:scale(1.045)}.rs-card-number{position:absolute;left:14px;top:12px;font-family:"Bebas Neue";font-size:54px;color:#fff;text-shadow:2px 2px #000}.rs-card-copy{padding:18px;display:grid;gap:8px}.rs-card-copy h3{font-family:"Bebas Neue";font-size:38px;line-height:.9;margin:0}.rs-card-copy strong{color:var(--orange);font-family:"Space Mono"}.rs-lookbook{min-height:70vh;padding:110px 8vw;background:#171717;display:grid;align-content:center;justify-items:start}.rs-lookbook p{max-width:620px;line-height:1.65;color:rgba(255,255,255,.7)}.rs-videos{min-height:100vh;padding:100px 5vw;background:#000}.rs-video-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:5px;margin-top:24px}.rs-video-tile{position:relative;aspect-ratio:16/9;overflow:hidden;background:#111}.rs-video{width:100%;height:100%;object-fit:cover;filter:grayscale(1) contrast(1.2)}.rs-video-tile span{position:absolute;left:12px;bottom:10px;font-family:"Bebas Neue";font-size:42px}.rs-floating{position:fixed;z-index:80;right:22px;bottom:22px;border:1px solid var(--orange);background:#000;color:#fff;padding:15px 22px;border-radius:999px;font-family:"Bebas Neue";font-size:19px;letter-spacing:.08em;opacity:0;transform:translateY(24px);transition:opacity .35s,transform .35s;cursor:pointer}.rs-floating.show{opacity:1;transform:none}.rs-modal{position:fixed;z-index:130;inset:0;background:rgba(0,0,0,.82);display:grid;place-items:center;padding:20px;opacity:0;visibility:hidden;transition:opacity .25s,visibility .25s}.rs-modal.open{opacity:1;visibility:visible}.rs-modal-card{position:relative;width:min(900px,96vw);max-height:88vh;overflow:auto;display:grid;grid-template-columns:1fr 1fr;gap:24px;background:#111;border:1px solid var(--orange);padding:22px}.rs-modal-card #rsClose{position:absolute;right:12px;top:10px;background:none;border:1px solid var(--line);color:#fff;width:38px;height:38px;cursor:pointer}.rs-modal-card img{width:100%;height:100%;max-height:520px;object-fit:cover}.rs-modal-card h2{font-family:"Bebas Neue";font-size:54px;line-height:.9;margin:12px 0}.rs-modal-card p{line-height:1.55;color:rgba(255,255,255,.72)}.rs-modal-card strong{display:block;color:var(--orange);font-family:"Space Mono";font-size:20px;margin:16px 0}.rs-modal-actions{display:flex;gap:10px;flex-wrap:wrap}@media(max-width:820px){.rs-nav{height:auto;min-height:66px;padding:10px 18px}.rs-nav div{display:none}.rs-nav strong{font-size:21px}.rs-nav a{padding:8px 10px}.rs-gallery-head{display:block}.rs-video-grid,.rs-modal-card{grid-template-columns:1fr}.rs-hero-copy p{letter-spacing:.12em}.rs-card{width:78vw}.rs-floating{left:16px;right:16px;border-radius:0}}@media(prefers-reduced-motion:reduce){html{scroll-behavior:auto}.rs-card-img,.rs-glitch-char,.rs-floating,.rs-teaser,.rs-preloader{transition:none}.rs-hero-video,.rs-video{animation:none}}';
}

function runtimeScript(o){
  return '(function(){var $=function(s){return document.querySelector(s)},$$=function(s){return [].slice.call(document.querySelectorAll(s))};var data=JSON.parse($("#rsData").textContent),products=data.products,track=$("#rsTrack"),modal=$("#rsModal"),runway=$("#rsRunway"),floating=$("#rsFloating"),progress=$("#rsProgress"),runwayTimer=null,runwayIndex=0,wishlist=[],cart=[];function money(v){try{return new Intl.NumberFormat(data.priceLocale||"es-ES",{style:"currency",currency:data.currency||"EUR"}).format(Number(v)||0)}catch(e){return (data.currency||"EUR")+" "+v}}function scrollToId(id){var el=document.getElementById(id);if(el)el.scrollIntoView({behavior:matchMedia("(prefers-reduced-motion: reduce)").matches?"auto":"smooth"})}$$("[data-scroll]").forEach(function(b){b.addEventListener("click",function(){scrollToId(b.dataset.scroll)})});function openProduct(i){var p=products[i];if(!p)return;var card=$$("[data-product-index]")[i],img=card&&card.querySelector("img");$("#rsModalMedia").innerHTML=img?\'<img src="\'+img.src+\'" alt="\'+p.name.replace(/"/g,"&quot;")+\'">\':"";$("#rsModalEyebrow").textContent=p.eyebrow||p.category||"";$("#rsModalTitle").textContent=p.name;$("#rsModalDescription").textContent=p.description||"";$("#rsModalPrice").textContent=money(p.price);$("#rsWishlist").onclick=function(){wishlist.push(p.id);toast("Añadido a deseos: "+p.name)};$("#rsCart").onclick=function(){cart.push(p.id);toast("Añadido al carrito demo: "+p.name)};modal.classList.add("open");modal.setAttribute("aria-hidden","false");$("#rsClose").focus()}function closeProduct(){modal.classList.remove("open");modal.setAttribute("aria-hidden","true")}$$("[data-open-product]").forEach(function(b){b.addEventListener("click",function(){openProduct(Number(b.dataset.openProduct))})});$("#rsClose").addEventListener("click",closeProduct);modal.addEventListener("click",function(e){if(e.target===modal)closeProduct()});document.addEventListener("keydown",function(e){if(e.key==="Escape")closeProduct()});function setRunway(i){var cards=$$("[data-product-index]");cards.forEach(function(card,n){card.classList.toggle("runway-active",n===i)});if(cards[i])cards[i].scrollIntoView({behavior:"smooth",inline:"center",block:"nearest"})}function stopRunway(){clearInterval(runwayTimer);runwayTimer=null;runway.textContent="'+esc(o.runwayLabel)+'"}function startRunway(){stopRunway();runway.textContent="'+esc(o.runwayStopLabel)+'";setRunway(runwayIndex);runwayTimer=setInterval(function(){runwayIndex=(runwayIndex+1)%products.length;setRunway(runwayIndex)},'+clamp(o.runwayInterval,1200,9000,2500)+')}runway.addEventListener("click",function(){runwayTimer?stopRunway():startRunway()});window.addEventListener("beforeunload",stopRunway);function tick(){var max=document.documentElement.scrollHeight-innerHeight;progress.style.width=(max>0?Math.min(100,scrollY/max*100):0)+"%";floating.classList.toggle("show",scrollY>innerHeight*.7);requestAnimationFrame(tick)}requestAnimationFrame(tick);floating.addEventListener("click",function(){scrollToId("section1")});function toast(text){var el=document.createElement("div");el.textContent=text;el.style.cssText="position:fixed;z-index:220;left:50%;bottom:26px;transform:translateX(-50%);background:#ff5c00;color:#050505;padding:12px 18px;font-family:Space Mono,monospace;font-size:12px";document.body.appendChild(el);setTimeout(function(){el.remove()},1800)}var teaser=$("#rsTeaser"),pre=$("#rsPreloader"),preText=$("#rsPreloaderText"),word="'+esc(o.preloaderText)+'",i=0;function type(){if(!preText)return;if(i<=word.length){preText.textContent=word.slice(0,i++);setTimeout(type,70)}else setTimeout(function(){if(pre)pre.classList.add("hide")},450)}setTimeout(function(){if(teaser)teaser.classList.add("hide");type()},'+clamp(o.teaserDuration,0,5000,1200)+')})();';
}
...
var DEFAULTS={
  ...
  heroSubtitle:'VOLUMEN 01 · INVIERNO 2025',
...
if(EP.StudioTemplateRegistry&&EP.StudioTemplateRegistry.register&&!EP.StudioTemplateRegistry.get('fashion-commerce-pro')){
  EP.StudioTemplateRegistry.register({
    ...
    description:'RUBIK SOTA — DISRUPCIÓN Custom PRO with source-faithful teaser, video hero, horizontal gallery and runway mode.',
    ...
    thumbnail:{label:'FC'},
    tags:['fashion','commerce','rubik-sota','custom-pro','source-faithful'],
    schema:build.schema,
