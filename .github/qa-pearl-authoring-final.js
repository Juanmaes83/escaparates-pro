...
(async()=>{
 ...
 const z1=before.author.z;await f.locator('#stage').hover();await p.mouse.wheel(0,520);await p.waitForTimeout(350);const z2=(await p.evaluate(()=>window.CasebookFashionStability.getState())).author.z;if(!(z2>z1+20))throw new Error(`zoom-out failed ${z1}->${z2}`);
